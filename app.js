...
async function sb(path, opts = {}) {
  ...
  const res  = await fetch(SB_URL + path, { method, headers, body });
...
async function getProfile(userId) {
  const d = await sb(`/rest/v1/profiles?id=eq.${userId}&select=*`);
  return d[0] || null;
}

/** Créer ou mettre à jour un profil */
async function upsertProfile(profile) {
  return sb('/rest/v1/profiles', {
    method: 'POST',
...
async function updateProfile(userId, data) {
  return sb(`/rest/v1/profiles?id=eq.${userId}`, {
    method: 'PATCH',
    body: data
  });
}
...
async function getLeaderboard(classe, country, scope) {
  let url = `/rest/v1/profiles?classe=eq.${encodeURIComponent(classe)}&order=xp.desc&limit=50&select=*`;
  if (scope === 'local') {
...
async function searchUsers(query) {
  return sb(`/rest/v1/profiles?username=ilike.*${encodeURIComponent(query)}*&select=*&limit=10`);
}

/** Récupérer les stats globales */
async function getGlobalStats() {
  try {
    const users = await sb('/rest/v1/profiles?select=id');
    return { users: users.length, duels: 0, quizzes: 0 };
  } catch (e) {
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  // Tenter de restaurer la session sauvegardée
  const saved = localStorage.getItem('eb_session');
  if (saved) {
    try {
...
async function doRegister() {
  ...
  try {
    ...
    SESSION = authData;
    localStorage.setItem('eb_session', JSON.stringify(authData));
    ...
    const msg = e?.msg || e?.error_description || e?.message || 'Erreur. Réessaie.';
    if (msg.toLowerCase().includes('email') || msg.toLowerCase().includes('already') || msg.toLowerCase().includes('registered')) {
  showErr('e-email', 'Email déjà utilisé ou en attente. Essaie de te connecter directement.');
} else {
